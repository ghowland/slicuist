...
The goal of efficiency should be to create logarithmic functions (<i>log(N)</i>) where possible, but this takes creating efficiencies at every level we have available, so that no aspect of the content addition (new UI or new content) takes as long or longer than a similar piece of work did when initially implemented.  With decreasing effort/time to add new UI and content to this system, we may not reach logarithmic reduction of work (depending on your definition of work units), but we will definitey be working with sub-linear amounts of effort and time, which is a huge advantage over super-linear with standard models of scaling content.
